...
def _exhausted_message(data):
    ...
    for detail in data.get("details") or []:
        ...
        if debug.get("reason") == "REASON_QUOTA_EXCEEDED":
            return (message or N_("Plan quota used up")).rstrip(".")
...
def parse_usage_response(status, body):
    ...
    if status == 200:
        if not isinstance(data, dict):
            return {"loggedIn": True, "error": N_("Kimi Code usage response could not be parsed")}
    ...
    if status in (401, 403):
        return {"loggedIn": True, "error": N_("Kimi Code login rejected — run kimi once to refresh it")}
    return {"loggedIn": True, "error": N_("Kimi Code %s") % http_error_text(status)}
...
def get_kimi_code_usage():
    ...
    if not token:
        return {"loggedIn": False, "error": N_("Kimi Code is not signed in — run kimi and /login")}
    expires = num(login.get("expires_at"))
    if expires > 1e11:  # milliseconds
        expires /= 1000
    if expires and expires <= time.time():
        return {"loggedIn": True, "error": N_("Kimi Code login expired — run kimi once to refresh it")}
